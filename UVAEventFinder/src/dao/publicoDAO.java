...
public class publicoDAO {
	...
	private Publico criarPublico(Cursor cursor) {
		Publico model = new Publico(cursor.getInt(cursor.getColumnIndex(DatabaseHelper.Publico._ID)),
	...
	public long salvarPublico(Publico publico) {
		...
		if (publico.get_id() != null) {
			return getDatabase().update(DatabaseHelper.Publico.TABELA, valores, "_id = ?",
					new String[] { publico.get_id().toString() });
		}
		return getDatabase().insert(DatabaseHelper.Publico.TABELA, null, valores);
	}

	public Publico buscarPublicoPorId(int id) {
		Cursor cursor = getDatabase().query(DatabaseHelper.Publico.TABELA, DatabaseHelper.Publico.COLUNAS, "id = ?",
